...
import { FOUNDATION_ADDRESS, RPC_ENDPOINT, FOUNDATION_CONFIG } from '@/config/foundation';
...
const withRetry = async <T>(
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes('429')) {
      console.warn(`[${operationName}] Rate limit hit, retrying in ${delayMs}ms. Retries left: ${retries}`);
      await delay(delayMs);
...
const processBatch = async <T, R>(
  ...
  for (let i = 0; i < items.length; i += batchSize) {
    ...
    console.log(`Batch ${batchNumber} completed: ${successfulResults}/${batch.length} successful`);
  ...
  console.log(`Batch processing completed: ${results.filter(r => r !== null).length}/${items.length} total successful`);
  return results;
};
    ...
    const fetchMembers = async () => {
      ...
      try {
        ...
        const signatures = await withRetry(
          () => connection.getSignaturesForAddress(foundationPubkey, { limit: 1000 }),
          3,
          1000,
          'Fetch Signatures'
        );
        ...
        const processTransaction = async (sig: { signature: string }) => {
          try {
            ...
            if (!parsedTransfer) {
              console.log(`No valid transfer found in transaction ${sig.signature.slice(0, 8)}`);
              return null;
            }

            console.log(`Successfully parsed transaction ${sig.signature.slice(0, 8)}:`, {
              from: parsedTransfer.from.slice(0, 8),
              amount: parsedTransfer.amount,
              isNative: parsedTransfer.isNative,
              tokenMint: parsedTransfer.tokenMint ? parsedTransfer.tokenMint.slice(0, 8) : 'none'
            });
        ...
        const transactions = await processBatch(signatures, 3, processTransaction, 500);
        console.log(`Processed ${transactions.filter(t => t !== null).length} valid transactions`);
        ...
        transactions.forEach((tx) => {
          ...
          try {
            ...
            if (isNative) {
              ...
              console.log(`Adding SOL donation: ${solAmount} from ${from.slice(0, 8)}`);
              ...
              if (tokenType && DECIMALS[tokenType]) {
                ...
                console.log(`Adding ${tokenType} donation: ${tokenAmount} from ${from.slice(0, 8)}`);
        ...
        const memberList = Array.from(memberMap.values())
          .sort((a, b) => new Date(b.lastDonation).getTime() - new Date(a.lastDonation).getTime());
