export default {
  title: [
    { content: "Pacote de expressão" },
    { content: " criação", status: true },
  ],
  text: "A criação de emoticons Web3 apoia os criadores a receber recompensas de $MINIDOGE através de suas obras.",
  hot: "Quente",
  new: "Novo",
  selectText: "Padrão",
  selectOptions: [],
  reward: {
    title: "Apoio",
    text: "Escaneie o código QR ou copie o endereço da carteira",
  },
  bindingWarningText: "Vincule seu endereço de carteira SOL para publicar seu trabalho",
  bindingWarningText2: "Vincular agora",
  ownedBy: "Pertence a",
  bindSOLWallet: "Vincular carteira SOL",
  income: "Renda",
  like: "Curtir",
  check: "Verificar",
  publish: "Publicar",
  limitText: {
    image: "Imagem: JPG/PNG, ≤5MB",
    gif: "GIF: ≤10MB",
    video: "Vídeo: MP4, ≤50MB, ≤30s",
    batchUpload: "Limite de upload em lote 10",
  },
  selectFile: "Selecionar arquivo",
  userName: "Nome de usuário",
  userNamePlaceholder: "Por favor, insira seu nome de usuário",
  userNameTip: "4-60 caracteres, suporta chinês, inglês, números, sublinhado e sinal de menos",
  SOLWalletAddress: "Endereço da carteira SOL",
  SOLWalletAddressPlaceholder: "Por favor, insira seu endereço da carteira SOL",
  addLink: "Adicionar link",
  telegramPlaceholder: "Por favor, insira o link do Telegram",
  twitterPlaceholder: "Por favor, insira o link do Twitter",
  verify: "Verificar",
};