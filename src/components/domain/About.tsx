...
const About = ({ ...props }) => {
  const { show } = props;
  return (
    <div className="w-full h-auto bg-lyllw/40 py-24 z-10 relative" id="about">
      <div
      ...
      >
        <div className="flex justify-between items-center gap-x-4 flex-col lg:flex-row py-4 ">
          <p className="text-primary text-4xl lg:text-6xl leading-[2rem] lg:leading-[5rem] bg-gradient-to-r  w-max pointer-events-none">
            BRETT
          </p>
          ...
          >
            ...
            <CSSTransition in={show} timeout={1000} classNames="about">
              <div className="flex flex-col justify-center gap-y-4 w-full lg:w-1/2 text-center lg:text-left">
                <h1 className="text-stroke-sm text-5xl">Who's Brett?!</h1>
                <p className="text-2xl opacity-60">
                  Brett is the legendary character from Matt Furie’s Boys' club
                  comic. He is a dancer and loves video games. Now he is living
                  on the BASE blockchain as a Fan tribute. He has become blue
                  mascot of the blue chain, BASE.
                </p>
