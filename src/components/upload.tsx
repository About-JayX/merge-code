...
export const Upload = ({ 
  ...
  const validateFile = async (file: File): Promise<{ valid: boolean; error?: string }> => {
    ...
    // 验证文件类型和大小
    if (
      ((fileType === "image/jpeg" || fileType === "image/png") && fileSizeInMB <= maxSizes.image!) ||
      ...
      if (fileType === "video/mp4") {
        ...
        if (videoDuration > maxVideoDuration) {
          return { valid: false, error: `视频时长不能超过 ${maxVideoDuration} 秒。` };
        }
      }
      return { valid: true };
    } else {
      ...
      return { 
        valid: false, 
        error: errorMessages[fileType as keyof typeof errorMessages] || "文件类型或大小不符合要求。"
      };
    }
  };
